...
class RemoteControliMotions():
    ...
    def __init__(self, study, participant, debug = True):
    ...
    def connect(self):
        ...
        while not self.status_imotions == 0:
            ...
            self.sock.sendall(status_query.encode('utf-8'))
    ...
    def start_study(self):
        ...
        self.sock.sendall(start_study_query.encode('utf-8'))
        if self.debug:
            start_study_recv = self.sock.recv(1024).decode('utf-8')
            print(f"iMotions started the study {self.study}.")
    ...
    def end_study(self):
        ...
        self.sock.sendall(end_study_query.encode('utf-8'))
        # Get and print response
        if self.debug:
            end_study_recv = self.sock.recv(1024).decode('utf-8')
            print(f"iMotions ended the study {self.study}.")

    def close(self):
        self.sock.close()
        self.status_imotions = None
        if self.debug:
            print("iMotions connection for remote control closed.")
...
class EventRecievingiMotions():
    ...
    def start_study(self):
        ...
        self.sock.sendall(start_study_marker.encode('utf-8'))
        if self.debug:
            print(f"iMotions recieved the start study marker\n{self.start_study_marker}")

    def end_study(self):
        end_study_marker = f"M;2;;;marker_end_routine;{self.time_stamp};D;\r\n"
        self.sock.sendall(end_study_marker.encode('utf-8'))
        if self.debug:
    ...
    def event_x_y(self, x, y):
        ...
        self.sock.sendall(values_imotions.encode('utf-8'))   
    ...
    def close(self):
        self.sock.close()
        if self.debug:
            print("iMotions connection for event recieving closed.")
