...
def explore_pupil(df: pl.DataFrame) -> pl.DataFrame:
    ...
    df = non_causal_median_filter_pupil(df, size_in_seconds=1)
    df = average_pupils(df, result_column="pupil")
    df = non_causal_decimate(df, factor=6)
    return df
...
def _get_blink_segments(
    ...
    indices are not preserved by the @map_participants decorator.
    """
    blink_segments_data = []
    for pupil in pupil_columns:
        ...
        if neg_ones.sum() == 0:
            logger.warning(
                f"No blinks found in {pupil} for participant_id {participant_id}"
            )
            continue
    ...
    blink_segments_df = pl.DataFrame(
        ...
        strict=False,
        orient="row",
    ).sort("participant_id", "start_timestamp")
...
@map_participants
def non_causal_median_filter_pupil(
    df: pl.DataFrame,
    size_in_seconds: int,
    pupil_columns: list[str] = ["pupil_r", "pupil_l"],
) -> pl.DataFrame:
