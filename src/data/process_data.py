...
def load_dataset(participant_config: ParticipantConfig, data_config: DataConfigBase) -> Data:
    file_path = (
    ...
    if isinstance(data_config, iMotionsConfig):
        file_path = (
            data_config.load_dir / participant_config.id / f"{data_config.name_imotions}.csv"
        )
...
def transform_dataset(data: Data, data_config: DataConfigBase) -> Data:
    ...
    if data_config.transformations:
        for transformation in data_config.transformations:
            ...
            logger.debug(
                "Dataset '%s' transformed with %s", data_config.name, transformation.__name__
            )
...
def transform_participant_datasets(
    ...
    if isinstance(data_configs[0], iMotionsConfig):
        ...
        logger.debug(
            "Participant %s datasets are now merged with trial information", participant_data.id
        )
...
def save_dataset(data: Data, participant_data: Participant, data_config: DataConfigBase) -> None:
