...
class RemoteControliMotions:
    ...
    def __init__(self, study="test", participant="dummy", age=20, gender="Female"):
        self.study = study
    ...
    def start_study(self, mode="NormalPrompt"):
        if self.connected:
            logger.info(
                "iMotions (dummy) started study %s for participant %s", self.study, self.participant
            )

    def end_study(self):
        if self.connected:
            logger.info(
                "iMotions (dummy) ended study %s for participant %s", self.study, self.participant
            )

    def abort_study(self):
        if self.connected:
            logger.info(
                "iMotions (dummy) aborted study %s for participant %s", self.study, self.participant
            )

    def export_data(self, path):
        logger.info(
            "iMotions (dummy) exported data for study %s for participant %s to %s",
            self.study,
            self.participant,
            path,
        )
...
class EventRecievingiMotions:
    ...
    def send_marker_with_time_stamp(self, marker_name):
        if self.connected:
            logger.info(
                "iMotions (dummy) received the marker %s at %s", marker_name, self.time_stamp
            )
